...
class CatalogReader(Loggin.base):
  ...
  def GetModels(self):
    for k in self.CatalogData.keys():
      ...
      if (str(k) == "3FGL") :
        model = self.CatalogData[k]['model']
...
#########################
  ...
  def ReadPLcorrected(self,key,model):
    # based on 2FHL reading, not bug free for the rest
    indice = self.CatalogData[key]['indice']
    if (str(model) == 'd11') :
      index  = self.CatalogData[key]['data'].field('Intr_Spectral_Index_D11')[indice]
      eindex = self.CatalogData[key]['data'].field('Unc_Intr_Spectral_Index_D11')[indice]
    if (str(model) == 'g12') :
      index  = self.CatalogData[key]['data'].field('Intr_Spectral_Index_G12')[indice]
      eindex = self.CatalogData[key]['data'].field('Unc_Intr_Spectral_Index_G12')[indice]
    flux   = self.CatalogData[key]['data'].field('Flux50')[indice]
    eflux  = self.CatalogData[key]['data'].field('Unc_Flux50')[indice]
    return [self.name,flux,eflux,index,eindex]
  ...
  def ReadPLExpCutoff(self,key):
    ...
    eflux  = self.CatalogData[key]['data'].field('Unc_Flux_Density')[indice]
    eindex = self.CatalogData[key]['data'].field('Unc_Spectral_Index')[indice]
    cut = self.CatalogData[key]['data'].field('Cutoff')[indice]
    dcut = self.CatalogData[key]['data'].field('Unc_Cutoff')[indice]
    if key == '1FHL':
      pivot *= 1e3
      flux  *= 1e-3
      eflux *= 1e-3
    return [self.name,flux,eflux,index,eindex,cut,dcut,pivot]
